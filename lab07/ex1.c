...
long long int sum_simd(int vals[NUM_ELEMS]) {
	...
	__m128i sum_vec = _mm_setzero_si128();
	    ...
	    __m128i filter = _mm_and_si128(tmp, mask);
            sum_vec = _mm_add_epi32(sum_vec, filter); 
        }
        int arr[4] = {0, 0, 0, 0};
        _mm_storeu_si128((__m128i *)arr, sum_vec);
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
	...
	for(unsigned int i = 0; i < NUM_ELEMS/16*16; i += 16){
	    ...
	    tmp0 = _mm_and_si128(tmp0, _mm_cmpgt_epi32(tmp0, _127));
